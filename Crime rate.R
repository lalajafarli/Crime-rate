...
for_vars <- c()
for (b in 1:length(features)){
  outvals <- boxplot(df[[features[b]]],plot=FALSE)$out
  if (length(outvals)>0){
    for_vars[b] <- features[b]
  }
}
...
for (a in for_vars){
outvals <- boxplot(df[[a]],plot=FALSE)$out
mean <- mean(df[[a]],na.rm = TRUE)}

o3 <- ifelse(outvals>mean,outvals,NA) %>% na.omit()
o1 <- ifelse(outvals<mean,outvals,NA) %>% na.omit()

val3 <- quantile(df[[a]],0.75,na.rm = TRUE)+1.5*IQR(df[[a]],na.rm = TRUE)
df[which(df[[a]] %in% o3),a] <- val3

val1 <- quantile(df[[a]],0.25,na.rm = TRUE)-1.5*IQR(df[[a]],na.rm = TRUE)
df[which(df[[a]] %in% o1),a] <- val1
